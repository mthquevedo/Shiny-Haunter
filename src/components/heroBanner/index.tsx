...
export function HeroBanner() {
    ...
    return (
        <div className="flex flex-col justify-center gap-px xsm:gap-2 md:gap-5 lg:gap-3 2xl:gap-5 bg-hero-banner bg-left xl:bg-center bg-cover w-full h-[9rem] xsm:h-[11rem] md:h-[18rem] lg:h-[16rem] 2xl:h-[20rem] rounded-xl px-4 md:px-6 truncate shadow-md">
            <div className="text-xl md:text-3xl lg:text-4xl xl:text-4xl 2xl:text-5xl text-zinc-100 font-bold">
                <p>{t("home.tool_for")}</p>
                <p>{t("home.all_trainers")}</p>
            </div>
            <div className="flex flex-col items-start justify-between gap-1.5 xsm:gap-3 lg:gap-3 2xl:gap-4">
                <div className="text-[0.625rem] xsm:text-sm md:text-lg xl:text-lg 2xl:text-xl text-zinc-100">
                    <p>{t("home.organize_collection")}</p>
                    <p>{t("home.next_to_hunt")}</p>
                </div>
                <Link
                    to="pokedex"
